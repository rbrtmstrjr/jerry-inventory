-- 0112 — Key transit write-offs on Philippine calendar day in the P&L.
--
-- fn_pnl_facts matched transit_writeoff rows by `created_at >= p_from::timestamptz`
-- — a DATE cast at the SERVER's UTC midnight — while every other term (sales,
-- losses, expenses) keys on the PH `business_date`/`expense_date`. Philippine
-- time is UTC+8, so a write-off made in the PH-morning / UTC-previous-day window
-- has a `created_at` that lands on the previous UTC day and dropped out of a
-- same-day (or month-boundary) P&L — undercounting transit shrinkage.
--
-- Fix: anchor the bounds to +08:00 (PH, no DST), so a UTC created_at is matched
-- by PH calendar day. Body is otherwise byte-identical to 0083 — only the two
-- transit_writeoffs date-bound lines change. The row-walk fallback in
-- lib/pnl.ts gets the identical +08:00 anchoring so both paths agree.
  ...
  select jsonb_build_object(
    ...
    'transit_writeoffs', coalesce((
      ...
      where sm.movement_type = 'transit_writeoff'
        and sm.created_at >= (p_from::text || 'T00:00:00+08:00')::timestamptz
        and sm.created_at <= (p_to::text || 'T23:59:59.999+08:00')::timestamptz
    ), 0)
  ) into v_result;
