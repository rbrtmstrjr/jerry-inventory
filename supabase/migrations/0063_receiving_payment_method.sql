-- 0063 — payment method + reference no. on the AT-RECEIVING payment
--
-- A receiving already stored HOW MUCH was paid up front (`amount_paid`) and the
-- resulting `payment_status`. It never stored HOW that money moved — cash,
-- cheque, bank transfer, GCash — nor a reference (cheque no., transfer ref).
-- The `supplier_payments` ledger already carries `method`/`reference_no` for
-- payments recorded LATER on Payables; this brings the same to the initial
-- payment, stored on the receiving header itself (parallel to `sales.payment_method`).
--
-- Descriptive only: the balance formula is
--   total_amount − amount_paid − Σ(supplier_payments)
-- and is untouched. Same value set as `supplier_payments.method`. Both columns
-- are nullable (null for an unpaid/on-credit receiving — no money moved yet, and
-- for every historical row). No backfill.

alter table public.receivings
  add column if not exists payment_method text
    check (payment_method is null
           or payment_method in ('cash', 'bank', 'gcash', 'check', 'other')),
  add column if not exists reference_no text;

-- ── fn_receive_stock — 0062 body + two params (method, reference) ────────────
-- Stored only when money actually moved (v_paid > 0) on a supplier receiving;
-- the supplier-less "Add product" path leaves them null (no payment). The arg
-- list grows by two (both defaulted), so drop the old 9-arg signature first.
drop function if exists public.fn_receive_stock(uuid, text, jsonb, jsonb, text, bigint, date, boolean, text);
...
revoke all on function public.fn_receive_stock(uuid, text, jsonb, jsonb, text, bigint, date, boolean, text, text, text) from public, anon;
grant execute on function public.fn_receive_stock(uuid, text, jsonb, jsonb, text, bigint, date, boolean, text, text, text) to authenticated;
