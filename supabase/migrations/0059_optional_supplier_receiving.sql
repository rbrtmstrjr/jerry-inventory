-- 0059_optional_supplier_receiving.sql — supplier becomes OPTIONAL on receiving.
-- ============================================================================
-- Brings back a direct "Add product / Add engine" without reversing the 0049
-- enforcement: creation still happens ONLY inside this SECURITY DEFINER
-- function (no INSERT grant on parts/engines/engine_models). A supplier-less
-- "Add product" is just a supplier-less receiving:
--
--   • p_supplier_id NULL → NO supplier/debt logic at all (no credit-limit
--     check, no payable, no alerts). The receivings header is settled and
--     zero-value: total_amount = 0, amount_paid = 0, payment_status = 'paid'.
--     Catalog rows are still created/reused inline (0048 + 0052), stock is
--     added, and a `received` movement is booked — so the reconciliation
--     invariant Σ movements = stock_levels holds exactly as today.
--   • p_supplier_id present → UNCHANGED (debt, credit limit, due date,
--     override). The Suppliers → Receiving flow is untouched.
--
-- Attribution: new_part / new_model already accept an optional
-- `preferred_supplier_id` (`coalesce(new.preferred_supplier_id, p_supplier_id)`)
-- — so picking a supplier on the Add form stamps it as the product's PREFERRED
-- supplier (price info) without ever creating a payable; reusing an existing
-- catalog row leaves its preferred supplier untouched.
--
-- Signature, guard (is_owner), inline creation, reuse-by-barcode/SKU, JM
-- barcode minting, and the price>cost validation are all byte-identical to
-- 0053. Only the tail (payment/debt resolution) gains the supplier-less
-- short-circuit. `receivings.supplier_id` is already nullable — no schema
-- change; a `create or replace` on the same signature, safe on live data.
-- ============================================================================
