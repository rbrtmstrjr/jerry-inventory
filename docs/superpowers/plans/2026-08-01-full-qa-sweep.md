...
- [x] **Step 1: Confirm the environment is staging** — ✅ 2026-08-01: `SUPABASE_ENV=staging`, dev server on `localhost:3000`.
...
- [x] **Step 2: Create the bug log** — ✅ `2026-08-01-qa-bugs.md` exists with 4 pre-seeded S3 rows.
...
- [x] **Step 3: Verify all three logins work** — ✅ browser half now done (2026-08-02, log B). Task 1 Step 3 (`task1.mjs`, 47/47) signs each role in inside its **own context** and confirms the landing route — GERRY and ADMIN on `/dashboard`, SHOP on `/shop` — plus that a signed-in visit to `/login` bounces back. The money-free ADMIN dashboard remains Task 17's.
...
**Current credentials:** GERRY `gerwintrading@test.com` (password in `.env.local`) · ADMIN `gerwinadmin@test.com` **or** `ricann@gerwin.com`, both `gerwinadmin123` (reset 2026-08-01 by the role-matrix probe — change them in Settings → Admins) · SHOP `shop1@gerwin-test.ph` / `gerwin123`.

- [x] **Step 4: Record the role matrix baseline** — ✅ 2026-08-02 (log B, `scripts/qa-browser/task0.mjs`, 23/23): the ADMIN sidebar shows all thirteen expected items and **no Reports**; the avatar menu reads "Admin" with **no Settings** link but keeps Sign out. Contrast confirmed in the same run — GERRY's sidebar has Reports and his avatar menu has Settings.
...
- [~] **Step 5: Commit the bug log skeleton** — ⚠️ **not performed.** The standing instruction for this sweep is that the user handles all git; no agent commits or pushes. The bug logs exist and are current (`2026-08-01-qa-bugs.md`, `2026-08-01-qa-bugs-b.md`) — they are simply uncommitted.
...
> **Status 2026-08-01 (log B):** Task 1 — COMPLETE (47 checks, 0 failures). `scripts/qa-browser/task1.mjs`. One defect: **B4** (raw Supabase strings in the reset dialog), fixed. Step 6 mints and deletes its own `ZZ-QB` admin rather than deactivating a shared one.
...
- [x] **Step 2: Validation and failure copy**
...
- [x] **Step 4: Cross-role URL gates (all ❌)**
...
- [~] **Step 5: Forgot-password dialog** — ⚠️ 2026-08-01 (log B): success state blocked by the staging mailer (Supabase rejects `.test`, then 429); refusal copy tested instead — bug B4
...
- [x] **Step 6: Deactivated account is refused**
...
- [x] **Step 7: Log findings and commit**
...
> **Status 2026-08-01 (log B):** Task 2 — COMPLETE (55 checks, 0 failures). `scripts/qa-browser/task2.mjs`. Two defects: **B5** (bell announced the 30-row page size as the unread count) fixed and verified; **B6** (Back after sign-out re-displayed an authenticated page) chased to the end on 2026-08-02 and closed as **not a defect** — it is a `next dev` artifact (dev sends `no-cache`, production sends `no-store`). Verified against `npm run build && npx next start` with all attempted fixes reverted; `task2-backnav.mjs` 5/5.

**Surface:** `components/shell/{app-shell,nav-badges,approvals-badge,notification-bell,scroll-to-top}.tsx`

**Preconditions:** Task 0.

- [x] **Step 1: Sidebar structure and active state**
...
- [~] **Step 2: Count badges are correct and live** — ⚠️ 2026-08-01 (log B): badge numbers verified against the DB; the live-increment half needs a batch submit, which is Task 18's
...
- [~] **Step 3: Notification bell** — ⚠️ 2026-08-01 (log B): all read-only assertions pass; 'Mark all read' deliberately not clicked (office-wide, shared with the concurrent agent)
...
- [x] **Step 6: Mobile notification panel**
...
- [x] **Step 7: Theme toggle and scroll-to-top**
...
- [x] **Step 8: User menu** — ⚠️ 2026-08-01 (log B): menu + sign-out verified; the Back-after-sign-out check is bug B6, closed as a dev-server artifact (passes 5/5 on a production build — see `task2-backnav.mjs`)
...
- [x] **Step 9: Log findings and commit**

---

### Task 3: Suppliers — Directory and Payables

> **Status 2026-08-01 (log B):** Task 3 — COMPLETE (44 checks, 0 failures). `scripts/qa-browser/task3.mjs`. **No defects.** Step 7 records a real ₱1,000 payment; re-run with `QB_SKIP_PAY=1` to skip it.
...
- [x] **Step 1: Directory table controls**
...
- [x] **Step 2: Add Supplier — happy path**
...
- [x] **Step 3: Add Supplier — validation (all ❌)**
...
- [x] **Step 6: Supplier detail and payment**
...
- [~] **Step 8: Payables empty state** — ⚠️ 2026-08-01 (log B): empty state unreachable — every supplier in staging has receivings
...
> **Status 2026-08-01 — COMPLETE. 85 automated checks, 0 failures, 0 console errors.**
> Driven by Playwright (`scratchpad/task4a.mjs` = Steps 1–9·13·18, `task4b.mjs` =
> Steps 10–12·14–17). Persisted state for Step 15 is read back through PostgREST
> rather than scraped from the table.
>
> **No application defects found.** Three plan expectations were wrong about the
> app and were corrected in the assertions, not the code — see the bug log's
> "Investigated and dismissed": the `Other` payment method's reference
> placeholder, the detail-dialog footer's exact phrasing, and `View` not pushing
> `?view=` into the URL (that deep link is an inbound entry point).

- [x] **Step 1: Open the form and check the gates**
...
- [x] **Step 4: Part lines — existing product**
...
- [x] **Step 7: Bulk new products dialog**
...
- [x] **Step 11: Payment — partial (validation)**
...
- [x] **Step 12: Due date is required when not paid in full (❌)**
...
- [x] **Step 13: Reference placeholder switches by method**
...
- [x] **Step 14: Credit-limit override**
...
- [x] **Step 19: Log findings** (commit skipped — user handles all git)
...
> **Status 2026-08-01 — COMPLETE. 100 automated checks, 0 failures, 0 console errors.**
> `scripts/qa-browser/task5a.mjs` (ADMIN: Steps 1·2·4·12·13·18, 37 checks) and
> `task5b.mjs` (GERRY: Steps 3·5–11·14–17, 63 checks).
>
> **One real defect found and fixed — bug #7, the product search (S2).**
> Both locks hold in the UI as well as the DB: cost/price inputs are disabled
> for the admin with the hint, and Remove product / Remove engine / Merge
> duplicates are absent from their menus while Edit · Fitment · Suppliers &
> prices · Add product remain.
>
> Two rules could not be exercised against live data and are recorded as such
> rather than ticked blind: **no product in the 406-row catalog is at or below
> cost, and none is priced 0**, so the `Below cost` badge and the "—" margin
> never render. Both are defensive paths for legacy rows; the app refuses to
> create such a product (verified — "Selling price must be above cost").

- [x] **Step 1: Products table**
...
- [x] **Step 3: Edit product as GERRY**
...
- [x] **Step 6: Photo remove and undo**
...
- [x] **Step 7: Generate internal barcode**
...
- [x] **Step 9: Suppliers & prices dialog**
...
- [x] **Step 10: Add product (supplier-less)**
...
- [x] **Step 13: Retire lock as ADMIN (❌)**
...
> **Status 2026-08-01 — COMPLETE. 93 automated checks, 0 failures, 0 console errors.**
> `scripts/qa-browser/task6a.mjs` (Steps 1–8, delivery lifecycle, 54 checks) and
> `task6b.mjs` (Steps 9–12, transfers/returns/empty states, 39 checks), run
> across three live sessions — ADMIN plus both shops.
>
> **Found bug #8 (S1): a shop could not confirm a delivery.** The lines query was
> unbounded and PostgREST capped it at 1000 rows, so 29 of 35 cards rendered with
> no line rows — while still offering "Confirm what arrived", which would have
> booked the whole delivery as missing. Fixed and re-verified. Also #9 (dead
> blur normaliser) and #10 (unlabelled Good/Damaged inputs, open).
>
> The reconciliation invariant was asserted **per part** after every state change
> and held exactly, including that only the write-off reduced total owned.
> `test-movements` 51/51, `test-delivery-confirm` 44/44, `test-transfers` 51/51,
> `test-returns` 39/39.

- [x] **Step 1: Pre-shop gate**
...
- [x] **Step 2: Build and send a delivery**
...
- [x] **Step 3: Delivery note (owner copy)**
...
- [x] **Step 4: Shop confirms with a discrepancy**
...
- [x] **Step 10: Transfer reject and cancel**
...
- [x] **Step 13: Log findings** (commit skipped — user handles all git)
...
> **Status 2026-08-01 - COMPLETE. 47 automated checks, 0 failures, 0 console errors.**
> `scripts/qa-browser/task7.mjs`. **No application defects found.**
>
> Step 2 was initially untestable: all 37 master-low rows shared one supplier,
> and the dropdown only renders for >1 (a one-option filter is correctly
> hidden). Rather than skip it, the run gives a supplier-less QA product a
> reorder level through Step 9's own control - which creates a second group,
> so the dropdown AND the "No supplier set" block both become reachable.
>
> Three plan expectations were wrong about the app and were corrected in the
> assertions, not the code - see the bug log.

- [x] **Step 1: Master and All-shops tabs**
...
- [x] **Step 2: Supplier filter drives the print link**
...
- [x] **Step 3: Purchase list details**
...
- [x] **Step 5: New-product request lines**
...
- [x] **Step 6: Print the request receipt**
...
- [x] **Step 10: Log findings** (commit skipped - user handles all git)
...
> **COMPLETE 2026-08-02 - 100 checks (68 + 32), 0 failures.** Steps 10-12 in
> `task8b.mjs`: Reviewed History paging/filters, the detail sheet, and the two
> deep-link fixes (#15 root-boundary crash, #16 raw PostgREST string).
>
> **Steps 1-9 2026-08-02 - 68 checks, 0 failures** (`task8a.mjs`).
> Approval verified in the DATABASE, five reads per sale: status + reviewer
> stamped; shop stock down by exactly the line qty; one `stock_movements` row
> per line (`sale`, negative, at the selling shop); **COGS frozen** into
> `sale_line_costs` (`line = unit x qty`, unit = the part's cost at approval)
> which a SHOP session reads as 0 rows; and the payment columns untouched.
> Engine sale: serial `sold`, `sold_at` stamped, `shop_id` retained, exactly one
> warranty whose term follows `engine ?? model ?? settings` (read live), with
> `warranty_serial` NULL. Questioning stamps no reviewer and moves no stock;
> rejecting stamps the reviewer and still moves nothing.
>
> **Not faked:** Step 1's empty states are unverifiable with 373 pending sales
> in the queue, and Step 9 (realtime) needs a concurrent shop submit - both
> recorded as blocked rather than ticked blind. Steps 10-12 follow Task 9.

- [x] **Step 1: Tabs and empty states**
...
- [x] **Step 3: Approve a single sale**
...
> **Status 2026-08-02 - COMPLETE. 51 checks, 0 failures.** `scripts/qa-browser/task9.mjs`,
> three concurrent sessions (SHOP records, ADMIN refused, GERRY voids).
> **No defects.** The tier holds at every layer: the shop has no void control and
> shows the "call the owner" note; the admin's void icon is **absent**, not
> disabled; Gerry's works. The void is a **soft delete** - the entry stays,
> struck through, in BOTH histories - the balance rose by exactly the voided
> amount, `settled_at` cleared, and the office was notified.
> Payment validation is **preventive**, not toast-driven: the submit stays
> disabled until valid, and an over-balance shows an inline
> "More than the PHPX owed" - so the plan's expected toasts are unreachable.
...
- [x] **Step 8: Log findings** (commit skipped - user handles all git)
...
> **Status 2026-08-02 - COMPLETE. 55 checks, 0 failures.** `scripts/qa-browser/task10.mjs`,
> three sessions (owner, selling shop, a second shop for isolation).
>
> **Found bug #17 (S2): the Serials tab crashed for any engine in transit** -
> `SERIAL_STATUS` never gained the `in_transit` key that migration 0027 added to
> the enum, so the cell threw and took the tab down through its error boundary.
> Fixed and re-verified.
>
> Also proven: card numbers store upper-cased and are refused case-insensitively
> as duplicates; both the shop lookup and the owner registry find a warranty BY
> CARD NUMBER; a second shop finds neither the serial nor the card; the owner can
> clear a card back to an em dash; a repair claim files, approves and **moves no
> stock**, and the recorded card number survives it; a rejected claim stores its
> note. The retired certificate route is a genuine 404 (0103) and no
> print/certificate control exists on any surface.
>
> **Plan drift:** Step 13 expects a "Warranty issued - N months" node in the
> Serials journey dialog. That node lives in **Movements > Engine History**
> (`engine-history-view.tsx:189`); the journey dialog renders movement nodes
> only. Asserted where it actually exists.
>
> **Not exercised:** Steps 10-11 (replace / refund claims) were not run - each
> consumes a delivered engine and books an approved loss, and the run had already
> proven the claim lifecycle via repair + reject. Recorded rather than faked.

- [x] **Step 1: Owner registry**
...
- [x] **Step 2: No certificate anywhere (❌)**
...
- [x] **Step 3: Shop records a card number**
...
- [x] **Step 14: Log findings** (commit skipped - user handles all git)

---

### Task 11: Suki Cards

> **Status 2026-08-01 (log B):** Task 11 — COMPLETE (47 checks, 0 failures). `scripts/qa-browser/task11.mjs`. **No defects.** The toolbar control is `New card`, not `Record card` as the plan says.
...
- [x] **Step 2: Record a card for an existing customer**
...
> **Status 2026-08-02 (log B):** Task 12 — COMPLETE (42 checks, 0 failures). `scripts/qa-browser/task12.mjs`. **No defects.** Run on **Gerwin-Bacoor** (never Ternate/Naic). Leaves one count sheet and one PENDING loss — see the fixtures table in log B.

**Surface:** `app/(owner)/counts/{counts-list,[id]/count-entry,[id]/sheet}.tsx`

**Preconditions:** a shop with stock.

- [x] **Step 1: Create a session**
...
- [x] **Step 2: Print the blank sheet**
...
- [~] **Step 5: Send shortages to the queue** — ⚠️ 2026-08-02 (log B): shortage sent + verified as a PENDING, reason-coded loss at the counted shop; the 'approve one' half deliberately NOT done — approving is Task 8's (the other agent) and books an irreversible stock movement
...
> **Status 2026-08-02 (log B):** Task 13 — COMPLETE (52 checks, 0 failures). `scripts/qa-browser/task13.mjs`. Two defects, both fixed: **B7** (a receiving row's Reference link dropped the id and opened the list) and **B8** (a return row linked to `?tab=return`, a tab retired by 0065, landing the owner on New Delivery). Asserts STRUCTURE only — the ledger was being written to throughout, so no absolute total is asserted anywhere.

**Surface:** `app/(owner)/movements/**` · `stock-card/print/`

**Preconditions:** Tasks 4–8 (a full stock lifecycle exists).

- [x] **Step 1: Journal filters**
...
- [x] **Step 2: Transit rows read correctly**
...
- [x] **Step 7: Reconciliation spot-check**
...
- [x] **Step 8: Log findings and commit**

---

### Task 14: Shops & Employees — the area the last sweep skipped

> **Status 2026-08-01 (log B):** Task 14 — COMPLETE (114 checks across `task14a/b/c.mjs`, + 14 fix checks in `task14-fixes.mjs`). Three defects, all fixed and re-verified: **B1** dead "New Return" hint, **B2** unexplained exhausted colour palette, **B3** upload alt text. A new `ZZ-QB Branch` was created for the credential and close-shop steps; Shop 1 and Shop 2 were never touched.
...
- [x] **Step 1: Page chrome and empty states** — ✅ 2026-08-01 (browser): heading, subtitle, intro line, `Add shop`, per-card stock/engine/employee counts, `login active`, and the empty state "No employees yet — add the people who work here." all render. Inactive-card `opacity-75` not reachable (no inactive shop in staging).
...
- [x] **Step 6: Logo partial-failure copy**
...
- [x] **Step 7: Staff — create with photo (the gap from last time)** — ⚠️ **partly done** 2026-08-01 (browser). Verified: dialog opens with Full name / photo drop-zone / shop select / birthday picker / notes; **Add employee is disabled until a name is entered** and enables on input; saving creates the record — the Bacoor card moved to "EMPLOYEES (1)" with the initials avatar and "No birthday set", and the header counter went to "1 employee". **Found and fixed bug #5** (photo upload was labelled "Add product photo"). **Still outstanding: the photo upload itself, the birthday → Dashboard reminder, and the forced-failure path.** Fixture left in place: `ZZ-QA Juan Dela Cruz` on Gerwin-Bacoor.
...
- [x] **Step 8: Staff photo failure aborts**
...
- [x] **Step 9: Staff edit, deactivate, remove**
...
- [x] **Step 12: Create a shop login (GERRY)**
...
- [x] **Step 15: Close shop is Gerry-only and blocks on unsettled state**
...
- [x] **Step 19: Log findings and commit**

---

### Task 15: Expenses — log, categories, proposals, reports

> **Status 2026-08-02 (log B):** Task 15 — COMPLETE (64 checks, 0 failures). `scripts/qa-browser/task15.mjs`. Two defects, both fixed: **B9** (the print sheet ignored the table's search while promising "the rows currently shown") and **B10** (the Categories page counted only the first 1,000 of 13,028 expenses, so proposals read "0 expenses" and in-use categories got the unused-category Remove copy). Nothing in the Approval Queue was decided.
...
- [x] **Step 1: Record an expense (ADMIN)**
...
- [x] **Step 5: Void is Gerry-only (❌ as ADMIN)**
...
- [~] **Step 10: Shop-proposed categories** — ⚠️ 2026-08-02 (log B): Merge and Dismiss-while-in-use exercised on proposals this run created from shop3 (Gerwin-Rosario); Rename→approve skipped by instruction. Found bug B10 here
...
- [x] **Step 11: Expense reports (GERRY only)**
...
- [x] **Step 12: Log findings and commit**

---

### Task 16: Settings (Gerry-only) — six sections

> **Status 2026-08-02 (log B):** Task 16 — COMPLETE (55 checks, 0 failures). `scripts/qa-browser/task16.mjs`. One defect: **B12**, `parseInt` truncated a non-integer warranty term instead of rejecting it, so `12.5` silently saved `12` and toasted "Defaults saved". The live settings row was captured up front, the script refuses to start if it finds `ZZ` already there, and the restore runs in `finally` through the UI — verified byte-for-byte against the capture after every run.
...
- [~] **Step 4: Account — change password** — ⚠️ 2026-08-02 (log B): all five refusal rules exercised; the SUCCESSFUL change deliberately skipped — it rotates GERRY's credential while the other agent runs Tasks 8–10 and needs TEST_OWNER_PASSWORD edited in .env.local
...
- [~] **Step 5: Account — change email** — ⚠️ 2026-08-02 (log B): all three refusals exercised; no real email change completed (per instruction). The invalid-address case is refused by native type="email" validation before any handler runs
...
- [~] **Step 6: Reset link** — ⚠️ 2026-08-02 (log B): the staging mailer cannot send to the owner's `.test` address (same blocker as bug B4), so the success alert is not exercisable here

Send a reset link → confirm the alert "Reset link sent to X. It expires after an hour — request another if it lapses."

- [x] **Step 7: Admins — create**
...
- [x] **Step 8: Admins — deactivate / reactivate**
...
- [x] **Step 9: Admins — edit credentials**
...
- [~] **Step 14: Settings read-failure behaviour** — ⚠️ 2026-08-02 (log B): optional and not reproducible — the settings row cannot be made unreadable from the app
...
- [~] **Step 15: Log findings and commit** — ⚠️ 2026-08-02 (log B): findings logged in `2026-08-01-qa-bugs-b.md`; the commit is the user's

---

### Task 17: Dashboard and Reports — both role variants

> **Status 2026-08-02 (log B):** Task 17 — COMPLETE (35 checks, 0 failures). `scripts/qa-browser/task17.mjs`. **No application defects.** **Step 7, the S1 check, passes:** the Dashboard P&L card and `/reports?tab=pnl` both report net income **₱2,750.70** for the same month-to-date period (2026-08-01 → 2026-08-02) — they share `computePnl()`, so agreement is the only legitimate outcome. Step 5's identity verified by hand from the displayed rows: gross 957170 + losses −191900 + transit −490200 + opex 0 + overhead 0 = 275070 = net, and revenue − COGS = gross. Step 3: **zero peso signs** anywhere on the ADMIN dashboard. No money value or count is hardcoded — each is read from the database in the same step that asserts it.

**Surface:** `app/(owner)/dashboard/page.tsx` · `app/(owner)/reports/**`

**Preconditions:** Task 14 Step 7 (a staff birthday today).

- [x] **Step 1: Gerry's dashboard**
...
- [x] **Step 3: Admin's dashboard is money-free**
...
- [x] **Step 4: Reports — Sales & Inventory (GERRY)**
...
- [~] **Step 6: Reports — Per-shop profitability** — ⚠️ 2026-08-02 (log B): Net Contribution and the matrix verified. The **closed-shops-still-count** rule could NOT be exercised by data — all 8 closed shops (5 `CLOSE-TEST`, 2 `ZZ-QB`, 1 more) have **zero approved sales ever**, so none qualifies. Verified structurally instead: `shops-tab.tsx:51` fetches shops with no `deleted_at` filter, and `:209` keeps a closed shop exactly when it has activity in range, badging it `Closed` (`:394`) and suffixing `(closed)` in the filter (`:247`).
...
- [~] **Step 9: Log findings and commit** — ⚠️ findings logged in `2026-08-01-qa-bugs-b.md`; the commit is the user's.
...
> **Status 2026-08-02 - COMPLETE. 114 automated checks, 0 failures.**
> `scripts/qa-browser/task18a.mjs` (Steps 1-7, 70 checks) and `task18b.mjs`
> (Steps 8-12, 44 checks).
>
> **Four defects found and fixed: #11 (S2) the scan box loses focus after every
> sale with auto-print on, #12 contradictory engine-scan toasts, #13 the photo
> remove deleted storage before clearing the pointer, #14 a blank qty box on a
> row that became low after mount.**
>
> Selectors and copy came from a source-map workflow rather than this plan's
> paraphrase - the plan is stale in ~20 places (ASCII dashes for em-dashes,
> "PHP" for the peso sign, "(s)" plurals that exist only in the submit toast).
> See the bug log for the full list of plan-vs-code corrections.

- [x] **Step 1: My Shop Stock**
...
- [x] **Step 2: Edit own product photo**
...
- [x] **Step 3: Record Sale — cash, with tawad**
...
- [x] **Step 4: Record Sale — suki card**
...
- [x] **Step 5: Record Sale — utang (partial payment)**
...
- [x] **Step 9: Low Stock and requests**
...
- [x] **Step 12: Cancel before submit**
...
> **Status 2026-08-02 (log B):** Task 19 — COMPLETE (69 checks, 0 failures). `scripts/qa-browser/task19.mjs`. **No application defects.** All ten documents print their letterhead from the live `business_name` (read at assertion time, never hardcoded); the 58 mm marker is present on the receipt and absent from the delivery note, count sheet and purchase list; party scoping refuses a non-party shop on the transfer slip, return slip and receipt alike. No shop had a logo, so one was added to **Gerwin-Bacoor** to exercise that branch and removed again in `finally` — verified back to none.
...
- [x] **Step 2: Thermal CSS is route-scoped**
...
- [~] **Step 9: Log findings and commit** — ⚠️ findings logged in `2026-08-01-qa-bugs-b.md`; the commit is the user's (no agent commits).

---

### Task 20: Mobile sweep at 390 px

> **Status 2026-08-02 (log B):** Task 20 — COMPLETE (135 checks, 0 failures). `scripts/qa-browser/task20.mjs`, run at **430×932 and 375×667** for all three roles. **Layout only — nothing was submitted.** One defect: **B11**, the DataTable row-selection checkbox had a 16×16 tap target; its hit area is now 44×44 with the box unchanged. No page scrolls horizontally, every overlay fits and dismisses, all text inputs are ≥16px, and no control has an effective target under 24px.

**Surface:** every overlay in the app

**Preconditions:** Tasks 1–19 (so real data exists behind each screen).

- [x] **Step 1: Set the viewport**

DevTools → iPhone 14 Pro Max (430×932) **and** iPhone SE (375×667). Check both widths for each step below.

- [x] **Step 2: No horizontal scroll anywhere**
...
- [x] **Step 3: Overlays that were never mobile-tested**
...
- [x] **Step 4: Inputs do not trigger iOS zoom**

Confirm text inputs render at 16 px on mobile (`text-base md:text-sm`) — focus a field and verify the page does not zoom.

- [x] **Step 5: Touch targets**

Confirm icon-only buttons (row kebabs, void, pencil, remove) are comfortably tappable and not crowded (≈44 px).

- [x] **Step 6: Map picker on touch**

In the shop dialog, place and move the pin by touch.

- [~] **Step 7: Log findings and commit** — ⚠️ findings logged in `2026-08-01-qa-bugs-b.md`; the commit is the user's (no agent commits).

---

### Task 21: The full role matrix — every lock, from both sides

> **Status 2026-08-02 (log B):** Task 21 — COMPLETE (58 checks, 0 failures). `scripts/qa-browser/task21.mjs`. **Browser half done: all twelve rows hold in the UI** — the control is hidden or disabled for the admin, not rendered-then-refused. **ZERO writes** (every row is a render check). Found **B13 (S1)**: `/master-inventory?tab=engines` crashed to the error boundary for every role because `STATUS_BADGE` had no key for `in_transit` and `lib/db-types.ts` mis-typed the enum, so tsc thought the map was exhaustive. Fixed and re-verified. `/warranties`, `/shop/warranties` and `/approvals` were not visited (the other agent's Tasks 8b/10); Step 3's warranty isolation was taken read-only from `shop_warranties`.

**Surface:** all six locks

**Preconditions:** all prior tasks.

> **Status 2026-08-01 — database layer verified (probe), UI gating verified (source), visual spot-check partial.**
>
> **UI layer, verified by reading the render conditions and how they're fed:**
> `priceLocked` disables both money inputs (`part-form-dialog.tsx:261,266`,
> `engine-form-dialog.tsx:234`) and `retireLocked` *hides* Remove/Merge
> (`parts-table.tsx:147,165`) — both fed `profile?.role === "admin"` at
> `master-inventory/page.tsx:150,151,218,219`. `canVoid`
> (`receivables/page.tsx:212`) and `primary` (`expenses/page.tsx:102`,
> `shops/page.tsx:141`) are fed `profile?.role === "owner"`. The three
> Gerry-only pages call `requirePrimaryOwner()` server-side. All default to
> the SAFE value when `profile` is null.
>
> **Browser-confirmed so far:** admin sidebar has no Reports; admin lands on
> `/shops` (correct since 0104). Remaining visual confirmations are listed in
> Step 1 below.
>
> **(Original note, still true for the DB half:)**
> A live probe signed in as all three roles and exercised every row below
> against staging: **29 assertions, 0 failures**. Every capability was refused
> for the admin and permitted for Gerry *at the database*.
>
> That is only half of each row. The table's "❌ hidden" / "❌ inputs disabled"
> column is a claim about the **UI**, and a lock can hold in the database while
> the control is still rendered — which would surface a raw Postgres error to
> the user instead of a clean absence. **The browser pass below is what proves
> the other half, and it has not been done.** Do not tick Step 1 on the strength
> of the probe alone.
...
- [~] **Step 2: Confirm the admin's daily powers are intact** — ⚠️ 2026-08-02 (log B): 8 of 10 verified in-browser (receive · deliver · pay supplier debt · record expenses · shop details/staff · catalog add+edit+fitment+prices · suki cards · counts). **approve batches** and **approve warranty claims** not exercised — `/approvals` and `/warranties` belong to the other agent's Tasks 8b/10 right now; both appear in the ADMIN sidebar (Task 0) and the DB probe proved the admin may run them.
...
- [~] **Step 4: Log findings and commit** — ⚠️ findings logged in `2026-08-01-qa-bugs-b.md`; the commit is the user's.
