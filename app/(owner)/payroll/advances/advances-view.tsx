...
import { HandCoins, History, Loader2, Plus, Undo2, Wallet } from "lucide-react";
...
export function AdvancesView({
  ...
  // Reveal the cash-advance history in batches so a long ledger paints instantly.
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Primary action lives in a modal now — the tab is for reviewing. */}
      <div className="flex justify-end">
        <Button onClick={() => setGiveOpen(true)}>
          <Plus className="size-4" /> Give a cash advance
        </Button>
      </div>
      ...
      <div className="grid items-start gap-4 lg:grid-cols-2">
          ...
          <CardContent>
            {balances.length === 0 ? (
              <p className="py-6 text-center text-sm text-muted-foreground">
                No outstanding cash advances — everyone&apos;s settled.
              </p>
            ) : (
        ...
        {/* Cash advances given (history) */}
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center gap-2 text-base">
              <History className="size-4" /> Cash advances given
            </CardTitle>
            <CardDescription>
              Every cash advance recorded, newest first. Void a mistaken entry.
            </CardDescription>
          </CardHeader>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <HandCoins className="size-4" /> Give a cash advance
            </DialogTitle>
            <DialogDescription>
          ...
          <DialogFooter>
            ...
            <Button onClick={onGive} disabled={busy}>
              {busy ? (
      ...
      <ConfirmDialog
        ...
        title="Void this cash advance?"
        description="Use this for a mistaken entry. It's refused if the cash advance has already been deducted from a payslip."
        confirmLabel="Void"
        destructive
        onConfirm={async () => {
          if (!voiding) return;
