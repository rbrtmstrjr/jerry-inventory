...
import { approveLoss, approveSale, reviewSubmission } from "./actions";
...
export function ApprovalsView({
  ...
  // Near-live queue: refresh when any submission changes.
  React.useEffect(() => {
    const supabase = createClient();
    let timer: ReturnType<typeof setTimeout> | null = null;
    const refresh = (isNew: boolean) => {
      if (isNew) toast.info("New submission arrived");
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          {sales.length + losses.length} waiting — stock only moves when you
          approve. Updates live as shops submit.
        </p>
      </div>
      ...
      <section className="flex flex-col gap-3">
        <h2 className="flex items-center gap-2 text-lg font-semibold">
          <ShoppingCart className="size-5" /> Sales ({sales.length})
        </h2>
        {sales.length === 0 && (
      ...
      <section className="flex flex-col gap-3">
        <h2 className="flex items-center gap-2 text-lg font-semibold">
          <AlertTriangle className="size-5" /> Losses / Adjustments ({losses.length})
        </h2>
        {losses.length === 0 && (
          <p className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground">
            No losses waiting for approval.
          </p>
        )}
        {losses.map((l) => (
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {dialog?.action === "question"
                ? "The employee sees this note on their submission and can clarify or cancel."
