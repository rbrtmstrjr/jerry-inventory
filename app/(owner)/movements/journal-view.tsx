...
function sourceLink(r: JournalRow): { href: string; label: string } | null {
  ...
  // `?view=` opens that receiving's detail dialog — without it the row lands on
  // the LIST, throwing away the id it already knows.
  if (r.receiving_id)
    return { href: `/suppliers?tab=receiving&view=${r.receiving_id}`, label: "Receiving" };
  // There is no `return` tab — 0065 made returns shop-initiated, approved under
  // "Transfers & Returns". `?tab=return` would be silently ignored.
