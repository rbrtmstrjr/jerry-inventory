/**
 * Smoke test: real sign-in → role-gated pages on the running dev server.
...
const ownerCookie = await sessionCookie("owner@jerrysmarine.test", "Owner!Dev2026");
...
check("GET /shop → redirected to owner area", r.status === 307 && r.location?.includes("/dashboard"), `got ${r.status} ${r.location}`);
r = await get("/", ownerCookie);
check("GET / → /dashboard", r.status === 307 && r.location?.includes("/dashboard"), `got ${r.status} ${r.location}`);

console.log("Employee session:");
r = await get("/shop", empCookie);
check("GET /shop → 200", r.status === 200, `got ${r.status}`);
...
check("GET /dashboard → redirected to /shop", r.status === 307 && r.location?.includes("/shop"), `got ${r.status} ${r.location}`);

console.log("Signed out:");
r = await get("/dashboard");
check("GET /dashboard → /login", r.status === 307 && r.location?.includes("/login"), `got ${r.status} ${r.location}`);
